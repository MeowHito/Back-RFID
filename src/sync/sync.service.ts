...
export class SyncService {
    ...
    private async buildEventResolver(campaignId: string): Promise<EventResolver> {
        ...
        // Build category map: local eventId -> race distance/category shown on public pages.
        // Priority: campaign.categories(remoteEventNo) distance -> event.distance -> event category/name.
        const categoryByEventId = new Map<string, string>();
        const raceConfigByRemoteEventNo = new Map<number, string>();
        categoryMappings.forEach((cat: any) => {
            const remoteNo = this.parseNumericValue(cat?.remoteEventNo);
            if (remoteNo === null) return;
            const label = this.toSafeString(cat?.name) || this.toSafeString(cat?.distance);
            if (label) {
                raceConfigByRemoteEventNo.set(remoteNo, label);
            }
        });
        events.forEach((event: any) => {
            const eventId = String(event._id);
            const eventRemoteNo = this.parseNumericValue(event?.rfidEventId);
            if (eventRemoteNo !== null) {
                const raceConfigLabel = raceConfigByRemoteEventNo.get(eventRemoteNo);
                if (raceConfigLabel) {
                    categoryByEventId.set(eventId, raceConfigLabel);
                    return;
                }
            }
            const eventDistance = this.parseDistanceValue(event?.distance);
            if (eventDistance !== null) {
                categoryByEventId.set(eventId, `${eventDistance} KM`);
                return;
            }
            const fallbackName = this.toSafeString(event?.category ?? event?.name);
            if (fallbackName) {
                categoryByEventId.set(eventId, fallbackName);
            }
        });
        return {
    ...
    async previewRaceTigerData(
        ...
        if (!['info', 'bio', 'split', 'score', 'passedTime'].includes(type)) {
            throw new BadRequestException('type must be one of: info, bio, split, score, passedTime');
        }
    ...
    async importEventsFromRaceTiger(campaignId: string): Promise<any> {
        ...
        if (events.length > 0) {
            ...
            try {
                // Build timingPointDistances map: "eid:TpName" → distance in km
                const timingPointDistances = new Map<string, number>();
                for (const [eid, tps] of timingPointsPerEvent.entries()) {
                    for (const tp of tps) {
                        if (tp.km !== undefined && tp.km !== null) {
                            timingPointDistances.set(`${eid}:${tp.name}`, tp.km);
                        }
                    }
                }
                this.logger.log(`  timingPointDistances: ${timingPointDistances.size} entries`);
                const splitResult = await this.syncSplitTimingRecords(campaignId, timingPointDistances);
                syncResult.splits = splitResult;
                this.logger.log(`  Split sync: upserted=${splitResult.upserted}, errors=${splitResult.errors.length}`);
            } catch (splitErr: any) {
                this.logger.warn(`  Split sync failed: ${splitErr?.message}`);
                syncResult.splits = { error: splitErr?.message };
            }
    ...
    /**
     * Sync split/checkpoint timing records from RaceTiger /Dif/split into TimingRecord collection.
     * Creates per-runner, per-checkpoint timing entries with distance, netTime, gunTime, splitTime.
     */
    async syncSplitTimingRecords(
        campaignId: string,
        timingPointDistances: Map<string, number>,
    ): Promise<{ upserted: number; errors: string[] }> {
        const result = { upserted: 0, errors: [] as string[] };
        try {
            const campaign = await this.getSyncEnabledCampaign(campaignId);
            const eventResolver = await this.buildEventResolver(campaignId);
            const campaignEventsForSync = await this.eventModel
            ...
            // Pre-load all runners for lookup
            ...
            for (const eid of eidsToFetch) {
                try {
                    ...
                    const maxPages = 200;
                    for (let page = 1; page <= maxPages; page++) {
                        const { response, parsedBody } = await this.requestRaceTiger(campaign, 'passedTime', page, eid);
                        if (!response.ok || !parsedBody) break;
                        const rows = this.extractRowsFromPayload(parsedBody);
                        if (!rows.length) break;
                        if (page === 1) {
                            this.logger.log(`  Split sync EID=${eid ?? 'all'}: ${rows.length} rows on page 1`);
                        }
                        ...
                        for (const row of rows) {
                            ...
                            const sortOrder = this.parseNumericValue(row?.SortOrder ?? row?.sortOrder) ?? 0;
                                ...
                                updateOne: {
                                    ...
                                    update: {
                                        $set: {
                                            ...
                                            scanTime,
                                            order: sortOrder,
                                            ...(netTimeMs !== null && netTimeMs > 0 ? { netTime: netTimeMs, elapsedTime: netTimeMs } : {}),
     ...
     */
    private async fetchScoreBibSet(
        campaign: any,
        raceTigerEids: number[],
    ): Promise<Set<string>> {
            ...
            try {
                ...
                for (let page = 1; page <= maxPages; page++) {
                    ...
                    for (const row of rows) {
                        ...
                        if (athleteId) bibSet.add(athleteId);
                    }
                    if (totalFetched >= totalExpected) break;
                }
            } catch (err: any) {
                this.logger.warn(`fetchScoreBibSet EID=${eid ?? 'all'} error: ${err?.message}`);
            }
        }
                ...
                try {
                    ...
                    for (let page = 1; page <= maxScorePages; page++) {
                        ...
                        for (const row of rows) {
                            ...
                            // Determine event ID — always prioritize the ROW's EventId
                            // because RaceTiger may return mixed-event data regardless of eid param
                            const rtEventId = this.resolveRaceTigerEventIdFromBioRow(row);
                            let eventId: string | null = null;
                            if (rtEventId !== null) {
                                eventId = eventResolver.eventIdByRaceTigerEventId.get(rtEventId) || null;
                            ...
                            if (!eventId) eventId = eventResolver.fallbackEventId;
                            if (!eventId) continue;
                            // Fast in-memory lookup within the CORRECT event only (no cross-event fallback
                            // to prevent BIB collisions between different distances)
                            ...
                            if (netTimeRaw) {
                                ...
                                if (netStr && netStr !== '-' && netStr !== '0') {
                                    updateData.netTimeStr = netStr; // exact RaceTiger value
                                }
                                const netTimeMs = this.parseTimeToMs(netTimeRaw);
                            ...
                            if (gunTimeRaw) {
                                ...
                                if (gunStr && gunStr !== '-' && gunStr !== '0') {
                                    updateData.gunTimeStr = gunStr; // exact RaceTiger value
                                }
                                const gunTimeMs = this.parseTimeToMs(gunTimeRaw);
                            ...
                            const genderRank = this.parseNumericValue(
                                row?.NetTimeGenderPosition ?? row?.netTimeGenderPosition
                                ?? row?.GenderPosition ?? row?.genderPosition
                                ?? row?.GenderRank ?? row?.genderRank ?? row?.SexRank ?? row?.sexRank
                            );
                            ...
                            const categoryRank = this.parseNumericValue(
                                row?.NetTimeCategoryPosition ?? row?.netTimeCategoryPosition
                                ?? row?.CategoryPosition ?? row?.categoryPosition
                                ?? row?.CategoryRank ?? row?.categoryRank
                            );
                            if (categoryRank !== null && categoryRank > 0) updateData.categoryRank = categoryRank;
                            const categoryNetRank = this.parseNumericValue(
                                row?.NetTimeCategoryPosition ?? row?.netTimeCategoryPosition
                            );
                            if (categoryNetRank !== null && categoryNetRank > 0) updateData.categoryNetRank = categoryNetRank;
                            const ageGroupRank = this.parseNumericValue(
                                row?.NetTimeCategoryGenderPosition ?? row?.netTimeCategoryGenderPosition
                                ?? row?.CategoryGenderPosition ?? row?.categoryGenderPosition
                                ?? row?.AgeGroupRank ?? row?.ageGroupRank
                            );
                            if (ageGroupRank !== null && ageGroupRank > 0) updateData.ageGroupRank = ageGroupRank;
                            const ageGroupNetRank = this.parseNumericValue(
                                row?.NetTimeCategoryGenderPosition ?? row?.netTimeCategoryGenderPosition
                            );
                            if (ageGroupNetRank !== null && ageGroupNetRank > 0) updateData.ageGroupNetRank = ageGroupNetRank;
                            // ── Pace ──
    ...
    /**
     * Lightweight split-only sync — used by auto-sync scheduler.
     * Builds timingPointDistances from checkpoints already in DB, then syncs /Dif/split.
     */
    async syncSplitOnly(campaignId: string): Promise<{ upserted: number; errors: string[] }> {
        const checkpoints = await this.checkpointsService.findByCampaign(campaignId);
        const timingPointDistances = new Map<string, number>();
        for (const cp of checkpoints) {
            if (cp.kmCumulative !== undefined && cp.kmCumulative !== null) {
                timingPointDistances.set(`all:${cp.name}`, cp.kmCumulative);
            }
        }
     ...
     * RaceTiger is a Chinese timing system and returns PassTime in CST (UTC+8) without timezone offset.
     * On UTC servers, new Date("2025-03-22T08:30:00") would be interpreted as UTC, causing an offset.
     * By marking the string as +08:00, JS correctly converts to UTC, and Thai browsers (UTC+7)
     * display the correct local time (1 hour behind China).
     */
    ...
    private parseTimeToMs(value: unknown): number | null {
        ...
        // Already a number — use smart heuristic to distinguish seconds vs ms
        if (typeof value === 'number') {
            if (value <= 0) return null;
            // Values > 360000 are almost certainly milliseconds (> 6 min in ms, but would be 100+ hours in seconds)
            if (value > 360000) return value;
            // Values <= 360000 are treated as seconds (up to ~100 hours, covers ultra-marathons)
            return value * 1000;
        }
        const str = String(value).trim();
        if (!str || str === '-' || str === '0' || str === '00:00:00' || str === '0:00:00') return null;
        // Try HH:MM:SS(.mmm) or H:MM:SS(.mmm)
        const hmsMatch = str.match(/^(\d{1,2}):(\d{2}):(\d{2})(?:\.(\d+))?$/);
        if (hmsMatch) {
            const h = parseInt(hmsMatch[1], 10);
            const m = parseInt(hmsMatch[2], 10);
            const s = parseInt(hmsMatch[3], 10);
            const frac = hmsMatch[4] ? parseInt(hmsMatch[4].padEnd(3, '0').slice(0, 3), 10) : 0;
            return (h * 3600 + m * 60 + s) * 1000 + frac;
        }
        // Try MM:SS(.mmm) — two-part durations from RaceTiger
        const msMatch = str.match(/^(\d{1,3}):(\d{2})(?:\.(\d+))?$/);
        if (msMatch) {
            const m = parseInt(msMatch[1], 10);
            const s = parseInt(msMatch[2], 10);
            const frac = msMatch[3] ? parseInt(msMatch[3].padEnd(3, '0').slice(0, 3), 10) : 0;
            return (m * 60 + s) * 1000 + frac;
        }
        // Try pure numeric string
        const num = Number(str);
        if (!isNaN(num) && num > 0) {
            // Same heuristic as numeric type above
            return num > 360000 ? num : num * 1000;
        }
        return null;
    }
